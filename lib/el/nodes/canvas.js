...
function Canvas () { this.define(
	...
	function graphics (_w, _h, _mode) {
		...
		return node.getContext(this.mode);
	},
	/*
	...
	function build (_operations) {
		var c, operations, linesCalculator;
		...
		El.walkDown(c.node(), function(_node) {
			...
			switch (_node.nodeType) {
				...
				case document.ELEMENT_NODE:
					...
					if (_node.id) buffer = El.get(_node);
					if (buffer) {
						operations.push(buffer.draw(linesCalculator));
						return false;
					}
					break;
			}
		...
		if (this.delegate("canvas_shouldBuild", [this, operations]) === false) return Promise.reject("NO_BUILD");

		this.delegate("canvas_willBuild", [this, operations]);

		return c.render(operations)
			.then(function() {
				var parent = c.parent();

				c.delegate("canvas_didBuild", [c, operations])
...
define.call(CanvasDelegate = {},
	...
	/* capture all element nodes */
	function el_shouldCaptureElementNode () {
		return true;
	},

	function el_renderOperations (_canvas, _node, _operations, _time) {
